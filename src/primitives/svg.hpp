...
namespace elements {
    ...
    struct SVGProcessor {
        ...
        Atomized atomize(Fragment<S>& fragment, Constraints&, SharedDescriptor& shared, SVGDescriptor& desc) {
            if (!desc.path.empty()) {
        ...
        LayoutState layout(Fragment<S>& fragment, Constraints& constraints, SharedDescriptor& shared, SVGDescriptor& desc, Atomized& atomized, const SizeResult& sizeResult) {
        ...
        Atomized postLayout(Fragment<S>& fragment, Constraints&, SharedDescriptor& shared, SVGDescriptor& desc, Atomized& atomized, L& layout) {
            std::vector<Atom> atoms {};
        ...
        template <LayoutStateType L>
        Placed place(Fragment<S>& fragment, Constraints& constraints, SharedDescriptor& shared, SVGDescriptor& desc, Atomized& atomized, L& lr) {
        ...
        Finalized<U> finalize(Fragment<S>& fragment, Constraints& constraints, SharedDescriptor& shared, SVGDescriptor& desc, Atomized& atomized, L& layout, Placed& placed) {
            ...
            fragment.fragmentStorage.atomsBuffer.write(ctx.frameIndex, atomPoints.data(), 6*sizeof(SVGPoint));
